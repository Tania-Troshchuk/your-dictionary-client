...
import { useDeleteWordMutation, useUpdateWordMutation } from "../../redux/services/wordsAPI"
import { Loader } from ".."
...
export const WordsTable = ({ words }: IProps) => {
  ...
  const [updateWord, { isLoading: isUpdateLoading, isSuccess: isUpdateSuccess }] = useUpdateWordMutation()
  ...
  const handleEditWord = useCallback(async (word: IWord) => {
    await updateWord({
      _id: word._id,
      word: word.word,
        ...
        <div className="grid grid-cols-[repeat(2,_minmax(0,_1fr)),_100px] border-x-4 border-b-4 rounded-b-lg break-words">
          {words.map(item => (
            <Fragment key={item._id}>
              <div className="border-t-2 border-r px-2 py-1 md:px-4">{item.word}</div>
              <div className="border-t-2 border-r px-2 py-1 md:px-4">{item.translation}</div>
              <div className="border-t-2 px-3 py-2 flex justify-between">
                <button onClick={() => handleSpeech(item.word)}>
                  <img src={voiceIcon} alt="voice" />
                </button>

                <button onClick={() => setModalAction({ word: item, action: 'edit' })}>
                  <img src={editIcon} alt="edit" className="w-[18px] h-[18px]" />
                </button>

                <button onClick={() => setModalAction({ word: item, action: 'delete' })}>
